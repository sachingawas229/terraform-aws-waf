...
variable "resource-arns" {
  type        = list(string)
  description = "List of resource ARNs to associate with the web ACL. ARNs be be for an ALB or an Amazon API Gateway stage"
  default     = []
}

variable "enable_web_acl_association" {
  description = "enable/disable web acl association"
  type        = bool
  default     = true
}